...
class PointsController {
	...
	@discardableResult func create(id: Int32,
								   startDate: Date,
								   endDate: Date,
								   totalPoints: Int32) -> Points {
		let points = Points(id: id, startDate: startDate, endDate: endDate, totalPoints: totalPoints, context:  CoreDataStack.shared.mainContext)
		CoreDataStack.shared.save()
		//put(chore: chore!)
		return points
	}

	func update(points: Points, startDate: Date, endDate: Date, totalPoints: Int32) {
		points.startDate = startDate
		points.endDate = endDate
		points.totalPoints = totalPoints
		CoreDataStack.shared.save()
		//put(representation: user.userRepresentation)
	}

	func delete(points: Points) {
		CoreDataStack.shared.mainContext.delete(points)
		CoreDataStack.shared.save()
